...
#define FPGA_CONFIG_DONE 5      // Input 
#define FPGA_CONFIG_CCLK 6      // Output 
#define FPGA_CONFIG_INITn 7     // Input
#define FPGA_SYSTEM_RSTn 20    // Output 
#define FPGA_CONFIG_CSn 21      // Output

// 8bit DataPath to the FPGA
// The Path is already inverted - See FPGA Doc to understand
...
// Input
#define F256K2_CONTEXT_SW0 22
#define F256K2_CONTEXT_SW1 23
...
int main() {
    ...
    if ( !gpio_get(F256K2_CONTEXT_SW0) && !gpio_get(F256K2_CONTEXT_SW1)) {
    ...
    if ( !gpio_get(F256K2_CONTEXT_SW0) && gpio_get(F256K2_CONTEXT_SW1)) {
            printf("Opening CNTX3/f256k2t9.bin\n");
            fr = f_open(&fil, "CNTX3/f256k2t9.bin", FA_READ); 
    }        

    if ( gpio_get(F256K2_CONTEXT_SW0) && gpio_get(F256K2_CONTEXT_SW1)) {
            printf("Opening CNTX4/foenix138.bin\n");
    ...
    if ( FR_OK == fr) {
        ...
        f256k2_Init_Prg_FPGA(); // This is getting the FPGA Ready to receive the Stream of Data (That will be sent by CORE1)
        ...
        do {
            // GO Read Buffer0 - 32K
            f_lseek(&fil, k);
            ReadStat = f_read (&fil, Buffer0, BUFFER_SIZE, &j);      // J = How many were read
            MailBox = multicore_fifo_pop_blocking();        // So When the process is started, it is going to send me a Code             
            multicore_fifo_push_blocking( ( j & 0x0000FFFF ));  // Send to The Core One, the message that it needs to choose Buffer0 and the Count
            if (i)
                break;            
            k = k + j;  // 
            i = BUFFER_SIZE - j; 
            
            // Alright, the last time the Buffer Read was < 32K then it is the last one.
            f_lseek(&fil, k);
            ReadStat = f_read (&fil, Buffer1, BUFFER_SIZE, &j);      // J = How many were read
            MailBox = multicore_fifo_pop_blocking();        // So When the process is started, it is going to send me a Code             
            multicore_fifo_push_blocking( ( j & 0x0000FFFF ) | 0x80000000 );  // Send to The Core One, the message that it needs to choose Buffer0 and the Count
            if (i)
                break;
            k = k + j;  // 
            i = BUFFER_SIZE - j;   
        }
        while ( 1 );    // Process Blocks of 32K first
        
        gpio_set_dir(FPGA_SYSTEM_RSTn, GPIO_OUT);
        gpio_put( FPGA_CONFIG_CSn,1);          // Bring UP the ChipSelect
        for (k = 0; k < 1000; k++){
...
void f256k2_context_man_init_io( void ) {
    ...
    gpio_init(FPGA_SYSTEM_RSTn);   // Output 
...
void f256k2_Prg_Block_FPG( void ) {    
    ...
    multicore_fifo_push_blocking(0x20400138); // Just Send a Message to Indicate that the process is started

    while (1)
    {
        MailBox = multicore_fifo_pop_blocking();    // Wait for Main Thread to be ready
        Size = MailBox & 0x0000FFFF;                // We got our Size
        if ( MailBox & 0x80000000 )                 // We got out Buffer Number
            Buffer = Buffer1;
       else
            Buffer = Buffer0;
        ...
        multicore_fifo_push_blocking(0x55AA55AA); // Just Send a Message to Indicate that the process of Pushing 32K is done        
    }
