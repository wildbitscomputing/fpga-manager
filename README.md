...
`Wildbits*.{lz4,gz,bin}` images (the file name lookup is case-insensitive). If
...
`wildbits-2026-07-18.lz4` takes precedence over
`Wildbits-2026-05-29.lz4`.
...
The upper 8 MiB contains replaceable LZ4 blobs at fixed addresses (2 MiB each):
...
Three immutable golden LZ4 images are linked into the manager firmware below
...
Normal boot tries SD, then the corresponding replaceable flash slot, then its
embedded golden image. To bypass SD and replaceable flash, hold the active-low
...
not expose the firmware or embedded golden-image region.
